...
class Notifications {
    ...
    fun singleNotification() = runBlocking { // notification stays only when debug, not run
        `when`(postDao.getAll()).thenReturn(flowOf(dummyPostModelFromDb))
        `when`(zsmeService.searchPosts(anyString(), anyInt(), anyInt())).thenReturn(
            listOf(dummyPostEntitiyFromService)
        )
    ...
    private val dummyPostEntitiyFromService = postMapper.mapToEntity(
        PostModel(
            id = 27451,
            date = SimpleDateFormat(
                "yyyy-MM-dd'T'HH:mm:ss",
                Locale.US
            ).parse("2021-11-07T12:49:33")!!,
